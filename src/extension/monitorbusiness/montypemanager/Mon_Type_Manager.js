...
import Mon_TypeDispose from './Mon_TypeDispose.vue'
import Mon_TypeDisposeType from './Mon_TypeDisposeType.vue'
...
let extension = {
  components: {
    ...
    //新建、编辑弹出框扩展组件
    modelHeader: '',
    modelBody: Mon_TypeDispose,
    modelFooter: Mon_TypeDisposeType
  },
  ...
  methods: {
    ...
    getRow(data) {
      this.editFormFields["Data_Config"]= JSON.stringify(data) 
    },
    getTypeRow(data){
      this.editFormFields["Run_Config"]= JSON.stringify(data) 
    },
    initColumnButton() {
      //在表最后添加操作列
      this.columns.push({
    ...
    modelOpenAfter(row) {
      ...
      this.editFormOptions.forEach(item => {
        item.forEach(x => {
          //如果是编辑就将配置设为只读
          if (x.field == "Data_Config") {
              x.disabled=isEDIT
          }
          if (x.field == "Run_Config") {
            x.disabled=isEDIT
        }
        })
    })
