          ...
          <div v-else :class="{ 'form-item-extra': item.extra }">
            ...
            >
              <el-input
                ...
                style="flex: 1"
                v-model="formFields[item.field][0]"
                @input="validateInput($event,item.field)"
                clearable
              />
            ...
            <div class="form-extra" v-if="item.extra">
              ...
              <!-- 新增添加多个按钮 -->
              <a
                v-else-if="item.extra.list"
                v-for="(son,i) in item.extra.list"
                :key="i"
                :style="son.style"
                @click="son.click(item, formFields[item.field])"
              >
                <i v-if="son.icon" :class="son.icon" />
                {{ son.text }}
              </a>
              <a
...
export default defineComponent({
  ...
  methods: {
    //防注入 input 输入内容，防止注入脚本等危险字符
    validateInput(event,type) {
      // 移除可疑字符
      let spvalue =this.formFields[type].replace(/[<>"'`]/g, '');
      // 防止输入 SQL 注入等危险字符  以及alert 等关键字提示警告
      const dangerousPatterns =/([<>"'`;\\]|(select\s[\w|*]+\sfrom)|(insert\s+into)|(update\s+\w+\s+set)|(delete\s+from)|(drop\s+table)|(alert\(|on\w+=)|(exec\s+)|(truncate\s+table)|(union\s+select)|(--|\/\*|\*\/))/gi;
      if (dangerousPatterns.test(spvalue)) {
        this.formFields[type] = "";
        this.$message.warning("检测到非法输入");
      }
    },
    getColWidth(item) {
