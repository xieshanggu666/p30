        ...
        <el-form ref="form" :model="form" label-width="80px">
            ...
            <el-form-item label="传感器">
                <el-radio-group v-model="form.sensorType">
                    <el-radio :key="item.key + index" v-for="(item, index) in showData.ArranchorCableSensor"
...
<script>
// import { DASgetDeviceSensor } from "@/api/page/gis/detail"
...
export default {
    ...
    components: {
        chart: () => import('./chart-line-gradiograph_1.vue')
    },
    computed: {
        startTime: function () {
    ...
    methods: {
        PageInit() {
            DASgetDeviceSensor({
                ...
                this.showData.ArranchorCableSensor = res;
                if (this.showData.ArranchorCableSensor.length > 0) {
                    this.form.sensorType = this.showData.ArranchorCableSensor[0].sensorType;
                    this.GetDASpageMonitorData();
                }
            })
        },
