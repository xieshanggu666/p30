  ...
  <div id="gnssC_1">
    <el-form ref="form" :model="state.form" label-width="80px">
      <el-form-item label="日期范围">
        <el-date-picker   :shortcuts="state.shortcuts" v-model="state.form.datePicker.time" type="datetimerange" range-separator="至" start-placeholder="开始时间"
      ...
      <el-form-item label="显示方式">
        <el-radio-group v-model="state.form.radio">
          <el-radio :key="item.key + index" v-for="(item, index) in state.showData.radioGroup" :label="item.key">{{
            item.name
          }}</el-radio>
        </el-radio-group>
      </el-form-item>
    </el-form>
    <chart class="chart-box" :getParam="state.chartData" :id="'point-gnssC-1'"></chart>
  </div>
</template>
...
import chart from './chart-line-gnss.vue'


console.log(new Date(2000, 1, 1, 12, 0, 0),'new Date(2000, 1, 1, 12, 0, 0)');
...
function GetDASpageMonitorData() {
  const getParam = {
    pointNum: props.currentDevice.Point_Num,
    total: 50000,
...
const props = defineProps(["currentDevice", 'flag'])
  // 0 : 小时 1：天    2 ：月
