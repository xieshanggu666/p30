...
export default {
  ...
  mounted() {
    this.countantSize.width = document.getElementById("container2").offsetWidth;
  ...
  methods: {
    initMqtt() {
      ...
      this.client.on("connect", function () {
        console.log(" d4 连接成功 close....",_this.params.subscriptionUrl);
        _this.subscribe(_this.params.subscriptionUrl);
      });
      this.client.on("message", function (topic, message) {
        console.log("接受获取消息",topic);
        console.log("接受获取消息D4",message);
    ...
    recev(topic, message) {
      ...
      console.log("接收到的D4信息进行数据转换",message);
