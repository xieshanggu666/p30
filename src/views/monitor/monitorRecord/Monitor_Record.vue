<template>
  <div class="com-tree">
    ...
    <div class="right-table">
      ...
      <!-- 图表 -->
      <div class="egis" v-if="view == 2 && Type_Id != 10">
...
import MonitorRecordTree from "./monitor/MonitorRecordFilterTree.vue";
import moment from "moment";
import shortcuts from "@/uitils/shortcuts";
import chart from "./components/chart-line.vue";
import chartN from "./components/chart-line2.vue";
...
export default {
  components: {
    ...
    VolForm: VolForm,
    VolHeader: VolHeader,
    MonitorRecordTree: MonitorRecordTree,
    chart: chart,
    chartN: chartN,
  },
  computed: {
  ...
  methods: {
    ...
    onChange(treeId, type) {
      this.$nextTick(() => {
        ...
        this.text = treeId.nodeName + "—监测数据管理";
        this.columns.splice(0, this.columns.length - 3);
        // 点击树默认触发
        console.log("type1111111111", type, this.view);
    ...
    pointPosition(treeId) {
      this.options_id = treeId.nodeValue;
      this.getDisposeChart(this.options_id);
      this.ChartInit(this.options_id);
    },

    pointPositionList(treeId) {
      this.options_id = treeId.nodeValue;
      this.getDisposeChart(this.options_id);
      this.ChartInit(this.options_id);
    },
