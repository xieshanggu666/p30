      ...
      <div class="right-table">
        ...
        <div v-if="view == 2">
          <div id="operation-data" class="dark">
            <div class="right-box">
              <div style="margin-top:-20px">预警内容：{{content}}</div>
              <chart
                class="chart-box"
                :getParam="chartData"
                :height="600"
...
export default {
  ...
  methods: {
    ...
    openOperationW(treeId, newTime) {
      this.model.box = true;
      this.content=treeId.Content
      this.columns.splice(0, this.columns.length - 1);
       var datePart = newTime.substring(0, 10);
      let initialDate = datePart;
      let updatedDate = this.subtractDays(initialDate, 1);
      this.startTime = updatedDate + " " + "00:00:00";
       this.endTime = newTime;
      this.pointPositionLast(treeId, this.endTime, this.startTime);
      this.form.datePicker.time[0] = this.startTime;
      this.form.datePicker.time[1] = this.endTime;
    },
    ...
    pointPositionLast(treeId, newTime, oldTime) {
      this.options_id = treeId.Point_Num;
      this.getMonitorData(this.options_id, newTime, oldTime);
    ...
    getMonitorData(data, newTime, oldTime) {
      let url = `/api/Mon_Point_Manager/GetRunData?pointNum=${data}&startTime=${oldTime}&endTime=${newTime}&pageIndex=${this.currentPage}&pageSize=${this.pageSize}`;
    ...
    click() {
      if (this.view == 1) {
        this.getMonitorData(this.options_id,this.endTime, this.startTime);
      } else {
        this.getDispose(this.options_id);
        this.getMonitorDataW(this.options_id,this.endTime, this.startTime);
        this.ChartInit(this.options_id,this.endTime, this.startTime);
