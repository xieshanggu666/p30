...
export default {
  ...
  data() {
    return {
      xjcpostion:{x:'',y:''}, //弹窗信息体
      showweiz:false,  //是否显示弹窗
      click_point: null,
      c: null,
  ...
  methods: {
    async gisInit() {
      try {
        this.currentPro =   (await this.http.post("/api/Sys_Personal_Setting/getPageData", {order:"desc",page:1,rows:30,sort:"ID",wheres:"[]"}, "正在请求中·····")).rows[1];
    ...
    handler.setInputAction((click) => {
      ...
      if (pick && pick.id) {
        ...
        if (pointData.Group_Num) {
          ...
          //显示监测点详情
          this.xjcpostion=click.position;
          this.showweiz=true; 
          //进行监测点操作
